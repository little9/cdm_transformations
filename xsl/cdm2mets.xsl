...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!--
        These are parameters that are passed to the stylesheet that contain paths to the CDM and FITS XML files. 
        They are generated with their own stylesheets.
    --> 
    ...
    <!-- This is the XML file that lists all of the XML files that were generated by running the FITS script --> 
    <xsl:variable name="vFitsList" select="document($pFitsPath)"/>
    
    <!-- This is the XML file that is created from the ContentDM TSV file --> 
    <xsl:variable name="vCdmMetadata" select="document($pCdmPath)"/>
    
    
    <!-- 
      pCollectionId is the ID for the collection that you are working with. Example: asm0566, chc5298.        
    --> 
    ...
    <!-- End METS header section -->



    <!-- This is going to go through a list of items in a collection. They can be a single item or and objectRecord, which has multiple items. 
         The if statement below tests if the collection is an object record or a single item.
    -->
            ...
            <xsl:if test="$vCdmMetadata/collection/descendant::node()[name()='singleItem' or name()='objectRecord']">
                <xsl:for-each 
                    ...
                    <!-- Let's determine the id of the item we are dealing with. -->
                    
                    <!-- First let's check to see if the archive field for singleItems isn't blank. 
                         If it isn't blank, then the archive field is the id for a single item. 
                         Else if the item is a compound object, the ID will be either an object id or an object number
                    -->
                    ...
                    <!--  Descriptive medata section --> 
                    <mets:dmdSec>
                        <xsl:for-each select="$vId">
                            <xsl:attribute name="ID">  
                                
                                <!-- For each of the ids that we made earlier add DMD and remove the file extension if we need to -->
                                <xsl:choose>
                                    <xsl:when test=".=''">
                        ...
                        <!-- Stick a MODS record into the METS descriptive medata section. -->
                        ...
                        <!-- End the descriptive medata section --> 
                    ...
                    <!-- Here we have the descriptive metadata section for all of the compound objects items. -->
                    <xsl:for-each select="self::node()[name()='objectRecord']/following-sibling::pageRecord">
                        ...
                        <mets:dmdSec>
                            ...
                            <mets:mdWrap MDTYPE="MODS">
                                ...
                                <!-- Each of these items gets their own MODS record as well --> 
                                <mets:xmlData>                                                                 
                                    <mods:mods version="3.5">
                                        <xsl:call-template name="tModsMaker">
     ...
     <!-- Administrative metadata section -->
            ...
            <mets:amdSec>
                ...
                <!-- In the admin section, we'll go through all the FITS files.  --> 
                
                <!-- We don't want to create sections for mtf files so only create the section is it's not an MTF file. -->   
                <xsl:for-each select="$vFitsList/fitsFiles/fitsFile[not(contains(@filename,'.mtf'))]">
                    ...
                    <xsl:for-each select="document(@filename)/fits:fits">                        
                        ...
                        <!-- Create an ID with the AMD prefix using the FITS file name -->
                        <mets:techMD ID="{concat('AMD',substring-before(substring($vFitsFileName,4),'.'))}">
                            <mets:mdWrap MDTYPE="OTHER">
                                <!-- Wrap the FITS metadata for each object into the METS -->
    ...
    <!-- File section --> 
            ...
            <!-- File sections can have groups with organize the files into sets. In this stylesheet there are 2 file groups: 'master' and 'ocr' -->
            <mets:fileSec>
                <mets:fileGrp ID="FG1" USE="master">
                    
                    <!-- We are using the FITS Metadata file list to create this section. -->
                    <xsl:for-each select="$vFitsList/fitsFiles/fitsFile[not(contains(@filename,'.mtf'))]">
                        <xsl:variable name="vPosCount" select="position()"/>
                        <xsl:for-each select="document(@filename)/fits:fits">
                            <xsl:variable name="vFitsFileName"
					  select="fits:fileinfo/fits:filename"/>
                            ...
                            <xsl:variable name="vDate" select="fits:fileinfo/fits:created"/>                            
                            <mets:file>
                                <!-- Give the file an ID with the obj prefix.  -->
            ...
            <xsl:choose>
                <xsl:when test="$vCdmMetadata/collection/descendant::node()[name()='singleItem' or name()='objectRecord']">
                    ...
                    <!-- The structMap is the only required section in a METS document.   -->
                ...
                <xsl:otherwise>
                    ...
                    <!-- This is the structmap for the FITS files. -->
                    <mets:structMap ID="SM1" TYPE="physical">
                        <mets:div>
                            <xsl:attribute name="ORDER" select="'1'"/>
                            <xsl:attribute name="ID" select="'SM1.1'"/>
    ...
    <!-- Begin MODS template -->
    <!-- The style sheet is embedding a MODS record for each of the items in the descriptive metadata section. --> 
    ...
    <xsl:template name="tModsMaker" exclude-result-prefixes="#all">
        ...
        <xsl:for-each select="field[@name='translatedtitle'][.!='']|field[@name='alternatetitle'][.!='']|field[@name='alternativetitle'][.!='']">
            <!-- The contentDM metadata has multiple fields that could be used for an alternate title in MODS  -->
